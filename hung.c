...
void BitSet_unset(BitSet self, size_t i);
ssize_t BitSet_any(BitSet self) __attribute__((pure));
...
BitSet new_BitSet(size_t size)
{
    ...
    self.limbs = (llong*)malloc(c * sizeof(llong));
    self.prev = (size_t*)malloc((c + 1) * sizeof(size_t));
    self.next = (size_t*)malloc((c + 1) * sizeof(size_t));
    *(self.first = (size_t*)malloc(sizeof(size_t))) = 0;

    size_t i;
    for (i = 0; i < c; i++)
    {
        *(self.limbs + i) = 0LL;
        *(self.prev + i) = *(self.next + i) = 0L;
    }
    *(self.prev + c) = *(self.next + c) = 0L;
 ...
 *
 * @param  self  The bit set
 * @param  i     The index of the bit to turn on
 */
void BitSet_set(BitSet self, size_t i)
{
    size_t j = i >> 6L;
    llong old = *(self.limbs + j);

    *(self.limbs + j) |= 1LL << (llong)(i & 63L);

    if ((!*(self.limbs + j)) ^ (!old))
    {
        j++;
	*(self.prev + *(self.first)) = j;
	*(self.prev + j) = 0;
	*(self.next + j) = *(self.first);
	*(self.first) = j;
    }
}
...
void BitSet_unset(BitSet self, size_t i)
{
    ...
    *(self.limbs + j) &= ~(1LL << (llong)(i & 63L));
    ...
    {
        j++;
	size_t p = *(self.prev + j);
	size_t n = *(self.next + j);
	*(self.prev + n) = p;
	*(self.next + p) = n;
	if (*(self.first) == j)
	    *(self.first) = n;
 ...
 */
ssize_t BitSet_any(BitSet self)
{
    if (*(self.first) == 0L)
        return -1;

    size_t i = *(self.first) - 1;
    return (ssize_t)(lb(*(self.limbs + i) & -*(self.limbs + i)) + (i << 6L));
}
